...
export function RecentCampaigns() {
  ...
  const [editForm, setEditForm] = React.useState({ name: "", description: "", body: "" });
  const [editLoading, setEditLoading] = React.useState(false);
  const [deleteLoadingId, setDeleteLoadingId] = React.useState<string | null>(null);

  // Mutations
  const updateCampaignMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: { name: string; description: string; body?: string } }) => {
  ...
  const handleEditSave = () => {
    ...
    setEditLoading(true);
  updateCampaignMutation.mutate({ id: editingCampaign.id, data: { name: editForm.name, description: editForm.description, body: editForm.body } });
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                placeholder="Edit your campaign description here..."
              />
            </div>
            <div className="space-y-2">
              <Label htmlFor="edit-campaign-body">Message/Email Body</Label>
              <ReactQuill
                theme="snow"
                value={editForm.body}
                onChange={(value: string) => setEditForm(f => ({ ...f, body: value }))}
                className="bg-white border border-blue-300 shadow-lg rounded-xl p-4 focus:outline-none"
                style={{ height: "250px" }}
                placeholder="Edit your campaign message or email body here..."
              />
            </div>
